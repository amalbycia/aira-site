...
/**
 * GreenSock's official `horizontalLoop` helper.
 *
 * Builds a seamless, infinitely-looping horizontal timeline from a set of
 * elements and (optionally) makes it draggable with momentum + snapping via
 * Draggable + InertiaPlugin. The returned timeline is augmented with:
 *   - `next(vars)` / `previous(vars)` — step one item
 *   - `toIndex(index, vars)` — go to an item by index (shortest direction)
 *   - `current()` / `closestIndex(setCurrent?)` — index bookkeeping
 *   - `draggable` — the created Draggable instance (when config.draggable)
 *
 * Source: GreenSock helper function (https://gsap.com/docs/v3/HelperFunctions),
 * ported to TS. The maths are unchanged — this is the proven core; our lightbox
 * UI wraps it. Draggable and InertiaPlugin must be registered by the caller.
 */
