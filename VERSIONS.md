...
## v3 — "Maroon Cinema" redesign

**Tag:** `v3-maroon-cinema` · **Branch:** `redesign-v3` (based on `redesign-v2`, so the
Page Content CMS work and every v2 bug fix carry over). Not deployed.

An awwwards-direction rebuild on the **live v1 palette** (maroon `#7a1f1f` · cream
`#f5ede0` · gold `#c9a96e`). Grotesk-led this time: Nohemi display/UI, Cormorant italic
reserved for the single accent word.

**Signatures**
- **Diptych hero** — the two brands ARE the hero: full-height Aira Photography /
  Agnitantra Events panels, each with its photo, brand type (SplitText char rise) and
  arrow button; hover swells one half. The client's two-button requirement, made physical.
- **Counter preloader** — 000→100 on a maroon curtain; the last 10 points only complete
  when fonts + the hero image are genuinely ready. Session-gated, 1.5s hard cap.
- **Filmstrip gallery** — desktop pins and pulls the photo strip horizontally (one
  transform, deterministic width via fixed aspect-ratios); mobile gets a calm vertical grid.
- **Custom cursor** — gold ring lerping after the pointer, swells over interactive targets.
