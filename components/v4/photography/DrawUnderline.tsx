...
/* Draw-random-underline — Osmo's technique, ported faithfully.
   (osmo.supply/demo/draw-random-underline; DrawSVGPlugin is free since
   GSAP 3.13, and 3.15 is installed.)

   Any element with [data-draw-line] and a child [data-draw-line-box] gets a
   hand-drawn squiggle underline on hover: one of six SVG variants is
   injected into the box, its stroke draws in with DrawSVG, and on leave it
   draws out through its own end before the SVG is removed. The variant
   index advances globally, so consecutive hovers anywhere on the page never
   repeat a squiggle.

   Box positioning/colour comes from app/v4.css (--draw-inset, --draw-bottom,
   --draw-color) so the effect never shifts layout. Strokes ride currentColor. */

gsap.registerPlugin(DrawSVGPlugin);

/* The six hand-drawn variants from the Osmo demo, verbatim. */
const SVG_VARIANTS = [
...
export default function DrawUnderline() {
  useEffect(() => {
    ...
    document
      .querySelectorAll<HTMLElement>("[data-draw-line]")
      .forEach((container) => {
        ...
        const onEnter = () => {
          ...
          if (nextIndex === null) {
            nextIndex = Math.floor(Math.random() * SVG_VARIANTS.length);
          }

          box.innerHTML = SVG_VARIANTS[nextIndex];
          ...
          // Advance for the next hover across all items.
